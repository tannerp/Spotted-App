...
class PostRepository {
  ...
  Future<List<Post>> fetchNewsFeed() async {
    final List<Post> result = await postApiClient.fetchNewsfeed(userRepo.user.token);
  ...
  Future<List<Post>> fetchMyPosts() async {
    final List<Post> result = await postApiClient.fetchMyPosts(userRepo.user.token);
    // return postApiClient .fetchNewsfeed();
    return result;
  }

  Future<String> createPost(Post post) async {
    return await postApiClient.createPost(userRepo.user.token, post);
  }
}
